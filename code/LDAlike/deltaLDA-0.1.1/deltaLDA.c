...
static PyObject * deltaLDA(PyObject *self, PyObject *args, PyObject* keywds)
{
  ...
  //
  static char *kwlist[] = {"docs","alpha","beta","numsamp",
  ...
  // Parse function args
  //
  if(!PyArg_ParseTupleAndKeywords(args,keywds,"O!O!O!ii|O!O!i",kwlist,
  ...
  model_params* mp;
  dataset* ds;
  if(ARGS_BAD == convert_args(docs_arg,alpha,beta,f_arg,init,&mp,&ds))
    {
...
PyMethodDef methods[] = 
  {
    {"deltaLDA", (PyCFunction) deltaLDA, 
     METH_VARARGS | METH_KEYWORDS, "Run DeltaLDA"},
    {NULL, NULL, 0, NULL}  // Is a 'sentinel' (?)
  };

// This is a macro that does stuff for us (linkage, declaration, etc)
PyMODINIT_FUNC 
initdeltaLDA() // Passes method table to init our module
{
  (void) Py_InitModule("deltaLDA", methods); 
